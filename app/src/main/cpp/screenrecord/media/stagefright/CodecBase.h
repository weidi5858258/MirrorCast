...
struct CodecBase : public AHandler, /* static */ ColorUtils {
    ...
    public:
        ...
        virtual void onComponentAllocatedExt(
                const char *componentName,
                const sp<AMessage> &pluginMsg) = 0;
        virtual void onComponentConfiguredExt(
                const sp<AMessage> &inputFormat,
                const sp<AMessage> &outputFormat,
                const sp<AMessage> &pluginMsg) = 0;
        // Mediatek Android Patch End
        // Mediatek Android Patch Begin, {for notify ouput format change in tunnel mode}
        virtual void onOutputFormatChangedExt(
                const sp<AMessage> &outputFormatchange) = 0;
        // Mediatek Android Patch End
    };
