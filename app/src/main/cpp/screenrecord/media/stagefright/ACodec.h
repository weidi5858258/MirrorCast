...
// Mediatek Android Patch Begin, {For sony muti-play}
class ACodecPluginBase {
public:
    ACodecPluginBase() {}
    virtual ~ACodecPluginBase() {}
    virtual int getCustomUsage() = 0;
    virtual status_t configure(const char *mime, const sp<AMessage> &msg) = 0;
    virtual void handleOMXDied() = 0;
    virtual uint32_t getCustomFlags(AString &mime) = 0;
    virtual uint32_t getCustomData(AString &componentName) = 0;
    virtual void prepareAllocateComponent(const sp<AMessage> &msg) = 0;
    virtual void beforeAllocateComponent(AString &name) = 0;
    virtual status_t afterAllocateComponent(AString &name, sp<IOMXNode> omxNode) = 0;
    virtual void setComponentAllocatedNotify(sp<AMessage> &notify) = 0;
    virtual void setComponentConfiguredNotify(sp<AMessage> &notify) = 0;
    // Mediatek Android Patch Begin, {For get sar-width/sar-height}
    virtual void getPixelRatio(sp<AMessage> &notify) = 0;
    // Mediatek Android Patch End

private:
    DISALLOW_EVIL_CONSTRUCTORS(ACodecPluginBase);
};

class ACodecPlugin : public ACodecPluginBase, public RefBase {
public:
    ACodecPlugin();
    virtual ~ACodecPlugin();
    virtual int getCustomUsage();
    virtual status_t configure(const char *mime, const sp<AMessage> &msg);
    virtual void handleOMXDied();
    virtual uint32_t getCustomFlags(AString &mime);
    virtual uint32_t getCustomData(AString &componentName);
    virtual void prepareAllocateComponent(const sp<AMessage> &msg);
    virtual void beforeAllocateComponent(AString &name);
    virtual status_t afterAllocateComponent(AString &name, sp<IOMXNode> omxNode);
    virtual void setComponentAllocatedNotify(sp<AMessage> &notify);
    virtual void setComponentConfiguredNotify(sp<AMessage> &notify);
    // Mediatek Android Patch Begin, {For get sar-width/sar-height}
    virtual void getPixelRatio(sp<AMessage> &notify);
    // Mediatek Android Patch End

private:
    void *mVendorLibHandle;
    ACodecPluginBase *mPlugin;

    void addACodecPlugin();
    void clearACodecPlugin();

    DISALLOW_EVIL_CONSTRUCTORS(ACodecPlugin);
};
// Mediatek Android Patch End
...
private:
    ...
    // Mediatek Android Patch Begin, {For sony muti-play}
    sp<ACodecPlugin> mPlugin;
    sp<AMessage> mPluginMsg;
    // Mediatek Android Patch End
    ...
    status_t setupMPEGCodec(bool encoder, int32_t numChannels, int32_t sampleRate);
