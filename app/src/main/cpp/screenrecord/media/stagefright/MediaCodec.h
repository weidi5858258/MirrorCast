...
class MediaCodecPluginBase {
public:
    MediaCodecPluginBase() {}
    virtual ~MediaCodecPluginBase() {}
    virtual void componentAllocated(const sp<AMessage> &msg) = 0;
    virtual void componentConfigured(const sp<AMessage> &msg) = 0;
    virtual void configure(sp<AMessage> &format, const sp<Surface> &surfaceTextureClient) = 0;
    virtual status_t setParameters(sp<AMessage> &params) = 0;
    virtual int getCustomUsage() = 0;

private:
    DISALLOW_EVIL_CONSTRUCTORS(MediaCodecPluginBase);
};

class MediaCodecPlugin : public MediaCodecPluginBase, public RefBase {
public:
    MediaCodecPlugin();
    virtual ~MediaCodecPlugin();
    virtual void componentAllocated(const sp<AMessage> &msg);
    virtual void componentConfigured(const sp<AMessage> &msg);
    virtual void configure(sp<AMessage> &format, const sp<Surface> &surfaceTextureClient);
    virtual status_t setParameters(sp<AMessage> &params);
    virtual int getCustomUsage();

private:
    void *mVendorLibHandle;
    MediaCodecPluginBase *mPlugin;

    void addMediaCodecPlugin();
    void clearMediaCodecPlugin();

    DISALLOW_EVIL_CONSTRUCTORS(MediaCodecPlugin);
};
...
struct MediaCodec : public AHandler {
    ...
    enum BufferFlags {
        ...
        // Mediatek Android Patch Begin, {add BLEL flag for atsc3.0}
        BUFFER_FLAG_BLEL          = 1024,
        // Mediatek Android Patch End
    };
    ...
    enum {
        SOFTWARE_DECODEER = 1,
        PLAYBACK_ON_VDP   = 2,
        PLAYBACK_ON_NUPLAYER = 4,
    };
    // Mediatek Android Patch End
    static const pid_t kNoPid = -1;
    static const uid_t kNoUid = -1;
    // Mediatek Android Patch Begin, {For sony muti-play}
    static uint32_t getCustomFlags(AString &mime);
    // Mediatek Android Patch End
...
private:
    ...
    // Mediatek Android Patch Begin, {For sony muti-play}
    sp<MediaCodecPlugin> mPlugin;
    bool mHaveKick;
    Mutex mKickLock;
    Condition mShutdownCompleted;
    // Mediatek Android Patch End
